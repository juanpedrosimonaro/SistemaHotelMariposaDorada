...
function BlogLayout({children}){
  ...
  useEffect(() => {
    ...
    (async()=>{
      ...
      //const metadatos = (await Promise.all(articleFiles.map(file=>context[file]()))).map(mod=>matter(mod.default));

      //const allFiles = (await Promise.all(allModules.map(mod=>axios(mod.default)))).map(res=>res.data)
  ...
  return (
    <motion.div 
      ...
      exit={{opacity:0}}
    >
      <nav className="w-screen h-[100px] text-white bg-black flex flex-row justify-center items-center relative" >
          <div className="font-playfair text-[35px] text-cl1 text-center">
            Mariposa Dorada
          </div>
          <span className="font-lato ml-[20px] text-[30px]">Blog</span>
          <Link to="/" className="absolute right-[20px]">Pagina Principal</Link>
      </nav>
      <main>
        <Outlet context={articlesMeta} /> 
      </main>
    </motion.div>
  )
}
